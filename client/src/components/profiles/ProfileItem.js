...
import { Card, Image, Label, List, Button, Icon, Header } from 'semantic-ui-react';
...
const ProfileItem = ({
  ...
  const followButton = !currentUserProfile ? null : currentUserProfile.following.find(
    ...
    <Button color="orange" circular size="tiny" onClick={() => follow(_id, name)}>
      Follow
    </Button>
  );
  ...
  const actionButtons = !currentUserProfile
    ...
    : recipientObj.status === 'requested' && (
        <>
          <Button color="teal" icon="check" onClick={() => acceptFriendRequest(_id)} />
          <Button color="red" icon="times" onClick={() => rejectFriendRequest(_id)} />
        </>
      );
  ...
  const statusMessage = (
    ...
    >
      ...
      {(requesterObj.status === 'accepted' || recipientObj.status === 'accepted') && 'Friend'}
    ...
    <Card raised>
      ...
      <Card.Content textAlign="center">
        <Button
          ...
          to={`/profile/${_id}`}
        />
        {auth.isAuthenticated && currentUser._id !== _id && requesterObj.status === 'accepted' && (
          <Button
            color="teal"
            size="tiny"
            circular
